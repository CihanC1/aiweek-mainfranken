...
public class AuthResource {
    ...
    public record UpdateProfile(@Size(min=2,max=80) String displayName,String preferredCity,Set<EventType> preferredEventTypes,AttendanceMode attendanceMode,Set<String> interests){}
    public record UserView(UUID id,String displayName,String email,String phoneNumber,boolean whatsappOptIn,String preferredCity,Set<EventType> preferredEventTypes,AttendanceMode attendanceMode,UserRole role,List<String> interests){
        static UserView of(AppUser u){return new UserView(u.id,u.displayName,u.email,u.phoneNumber,u.whatsappOptIn,u.preferredCity,u.preferredEventTypes,u.preferredAttendanceMode,u.role,UserInterest.<UserInterest>find("user",u).list().stream().map(i->i.tag).toList());}
    }
