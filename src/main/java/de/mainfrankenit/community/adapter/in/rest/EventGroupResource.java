...
public class EventGroupResource {
    ...
    static GroupView view(EventGroup g){var ids=EventGroupMember.<EventGroupMember>find("group",g).list().stream().map(m->m.user.id).toList();var messages=EventGroupMessage.<EventGroupMessage>find("group=?1 order by createdAt",g).list().stream().sorted(Comparator.comparing((EventGroupMessage m)->m.role!=ChatRole.SYSTEM).thenComparing(m->m.createdAt)).map(EventGroupResource::messageView).toList();return new GroupView(g.id,g.event.id,g.name,ids,messages);}
