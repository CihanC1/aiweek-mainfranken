...
class CoreServiceIntegrationTest {
    ...
    @Test @TestTransaction
    void newEventsNotifyOptedInUsers(){var user=users.create("Notify User");users.replaceInterests(user,Set.of("java"));users.optIn(user.id,"+491234567892",true);assertEquals(1,importer.upsert(draft("first","Würzburg")));Event event=Event.find("sourceUrl","https://integration.example/event/unique").firstResult();Notification notification=Notification.find("user=?1 and event=?2 and type=?3",user,event,NotificationType.MATCHING_EVENT).firstResult();assertNotNull(notification);assertTrue(notification.body.contains("passt zu deinen Interessen"));NotificationDelivery delivery=NotificationDelivery.find("notification",notification).firstResult();assertNotNull(delivery);assertEquals(DeliveryChannel.WHATSAPP,delivery.channel);assertEquals(DeliveryStatus.SENT,delivery.status);}
